...
C
C         w is the result.  size(w) = size(z) if nu is a scalar
C         or size(w) = [prod(size(z)), length(nu)] if nu is a vector.
C
...
C
      ...
      INTEGER NLHS, NRHS
C
      INTEGER MXCREATEFULL,MXGETPR,MXGETPI,MXISCOMPLEX
      COMPLEX*16 Z,W,W1,W2,WRK
      REAL*8 ZR,ZI,NU,MXGETSCALAR,CHECK,PI,D1MACH
      INTEGER FUN,KODE,UNFL,IERR,KOMPZ,KOMPW,MZ,NZ,LDBL
      INTEGER I,J,M,N
      INTEGER PNU,PNI,PZR,PZI,PWR,PWI,PERR
      DATA PI /3.1415926535897932385D0/
C
C     LDBL is the pointer increment for doubles
      DATA LDBL /8/
C
C     Pick up input arguments and allocate output array
C
      FUN = MXGETSCALAR(PRHS(1))
      N = MXGETM(PRHS(2))*MXGETN(PRHS(2))
      KOMPZ = MXISCOMPLEX(PRHS(3))
      KOMPW = KOMPZ
      IF (FUN.EQ.72 .OR. FUN.EQ.72*2) KOMPW = 1
      IF (N .EQ. 1) THEN
         MZ = MXGETM(PRHS(3))
         NZ = MXGETN(PRHS(3))
         PLHS(1) = MXCREATEFULL(MZ,NZ,KOMPW)
         IF (NLHS .GT. 1) PLHS(2) = MXCREATEFULL(MZ,NZ,0)
         M = MZ*NZ
      ELSE
         M = MXGETM(PRHS(3))*MXGETN(PRHS(3))
         PLHS(1) = MXCREATEFULL(M,N,KOMPW)
         IF (NLHS .GT. 1) PLHS(2) = MXCREATEFULL(M,N,0)
      ENDIF
      KODE = 1
...
C
      ...
      PWR = MXGETPR(PLHS(1))
      PWI = MXGETPI(PLHS(1))
      IF (NLHS .GT. 1) PERR = MXGETPR(PLHS(2))
C
...
C
      DO 200 J = 1, N
        DO 100 I = 1, M
          IERR = 0
C
C         Pick up and check one element of nu and z
C
          CALL MXCOPYPTRTOREAL8(PNU, NU, 1)
          IF (KOMPZ .EQ. 1) THEN
             CALL MXCOPYPTRTOREAL8(PNI, CHECK, 1)
             IF (CHECK .NE. 0.0D0) CALL MEXERRMSGTXT('NU must be real.')
          ENDIF
          CALL MXCOPYPTRTOREAL8(PZR, ZR, 1)
          ZI = 0.0D0
          IF (KOMPZ .EQ. 1) CALL MXCOPYPTRTOREAL8(PZI, ZI, 1)
          Z = DCMPLX(ZR,ZI)
C
C         Ai(z), Ai'(z), Bi(z) and Bi'(z)
C
          IF (FUN .EQ. 65) THEN
              CALL CAIRY(Z,IDINT(NU),KODE,W,UNFL,IERR)
          ENDIF
          IF (FUN .EQ. 66) THEN
              CALL CBIRY(Z,IDINT(NU),KODE,W,IERR)
          ENDIF
C
C         J_nu(z)
C
          IF (FUN .EQ. 74) THEN
             IF (NU .GE. 0.0D0) THEN
                CALL CBESJ(Z,NU,KODE,1,W,UNFL,IERR)
             ELSE IF (NU .EQ. DINT(NU)) THEN
                CALL CBESJ(Z,-NU,KODE,1,W,UNFL,IERR)
                IF (MOD(IDINT(NU),2) .NE. 0) W = -W
             ELSE IF (ZR .EQ. 0.0D0 .AND. ZI .EQ. 0.0D0) THEN
                W = -D1MACH(6)
             ELSE
                CALL CBESJ(Z,-NU,KODE,1,W1,UNFL,IERR)
                CALL CBESY(Z,-NU,KODE,1,W2,UNFL,WRK,IERR)
                W = DCOS(NU*PI)*W1 + SIN(NU*PI)*W2
             ENDIF
          ENDIF
C
C         Y_nu(z)
...
C
          IF (FUN .EQ. 89) THEN
             ...
             ELSE IF (NU .GE. 0.0D0) THEN
                CALL CBESY(Z,NU,KODE,1,W,UNFL,WRK,IERR)
             ELSE IF (NU .EQ. DINT(NU)) THEN
                CALL CBESY(Z,-NU,KODE,1,W,UNFL,WRK,IERR)
                IF (MOD(IDINT(NU),2) .NE. 0) W = -W
             ELSE
...
C
          IF (FUN .EQ. 73) THEN
             IF (NU .GE. 0.0D0) THEN
                CALL CBESI(Z,NU,KODE,1,W,UNFL,IERR)
             ELSE IF (NU .EQ. DINT(NU)) THEN
                CALL CBESI(Z,-NU,KODE,1,W,UNFL,IERR)
             ELSE IF (ZR .EQ. 0.0D0 .AND. ZI .EQ. 0.0D0) THEN
                W = -D1MACH(6)
             ELSE
                CALL CBESI(Z,-NU,KODE,1,W1,UNFL,IERR)
                CALL CBESK(Z,-NU,KODE,1,W2,UNFL,IERR)
                W = W1 - (2/PI)*SIN(NU*PI)*W2
             ENDIF
          ENDIF
C
C         K_nu(z)
...
C
          IF (FUN .EQ. 75) THEN
             IF (ZR .EQ. 0.0D0 .AND. ZI .EQ. 0.0D0) THEN
                W = D1MACH(6)
             ELSE
                CALL CBESK(Z,DABS(NU),KODE,1,W,UNFL,IERR)
             ENDIF
          ENDIF
C
...
C
          IF (FUN .EQ. 72 .OR. FUN .EQ. 72*2) THEN
             ...
             IF (ZR .EQ. 0.0D0 .AND. ZI .EQ. 0.0D0) THEN
                W = Z/Z
             ELSE IF (NU .GE. 0.0D0) THEN
                CALL CBESH(Z,NU,KODE,K,1,W,UNFL,IERR)
             ELSE IF (NU .EQ. DINT(NU)) THEN
                CALL CBESH(Z,-NU,KODE,K,1,W,UNFL,IERR)
                IF (MOD(IDINT(NU),2) .NE. 0) W = -W
             ELSE
                CALL CBESH(Z,-NU,KODE,K,1,W,UNFL,IERR)
                IF (K .EQ. 1) W = DCMPLX(DCOS(NU*PI),-DSIN(NU*PI))*W
                IF (K .EQ. 2) W = DCMPLX(DCOS(NU*PI),DSIN(NU*PI))*W
             ENDIF
          ENDIF
C
C         Illegal arguments. Return NaN.
          IF (IERR .EQ. 1) W = D1MACH(7)
C         Overflow.  Return Inf.
          IF (IERR .EQ. 2) W = D1MACH(6)
C         Some loss of accuracy in argument reduction.
          IF (IERR .EQ. 3) W = W
C         Complete loss of accuracy, z or nu too large.
          IF (IERR .EQ. 4) W = W
C         No convergence.  Return NaN
          IF (IERR .EQ. 5)  W = D1MACH(7)
C
          CALL MXCOPYREAL8TOPTR(DREAL(W), PWR, 1)
          IF (KOMPW .EQ. 1) CALL MXCOPYREAL8TOPTR(DIMAG(W), PWI, 1)
          IF (NLHS .GT. 1) CALL MXCOPYREAL8TOPTR(DFLOAT(IERR), PERR, 1)
C
...
C
      DOUBLE PRECISION DMACH(7)
      DATA DMACH /7*0.0D0/
      INTEGER MEXGETMATRIX,MXGETPR
C
C     First time through, get values from MATLAB.
C
      IF (DMACH(1) .EQ. 0.0D0) THEN
        CALL MEXEVALSTRING
     x  ('d1mach = [realmin, realmax, eps/2, eps, log10(2), Inf, NaN];')
        CALL MXCOPYPTRTOREAL8(MXGETPR(MEXGETMATRIX('d1mach')),DMACH,7)
      ENDIF
C
      D1MACH = DMACH(I)
      RETURN
