...
if nb_out>1
    ...
    %calcul derivees premieres
    dk=-(5.*long.^2.*xx./3+5*sqrt(5).*long.^3.*xx.^2.*sign(xx)./3).*etd;
...
if nb_out>2
    ...
    ddk=-5/3.*long.^2.*(1+sqrt(5)*abs(xx).*long-5*xx.^2.*long.^2).*etd;
    %ddk=-5./(3*long.^2).*(1+sqrt(5)*abs(xx)./long-5*xx.^2./long.^2).*etd;
    %(-5./(3*long.^2)-5*sqrt(5)./(3*long.^3).*abs(xx)-25./(3*long.^4).*xx.^2).*etd;
    ...
    error('Mauvais argument de sortie de la fonction matern52.m');
end