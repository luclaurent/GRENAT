...
% transform points to simulator space
[inFunc, outFunc] = getTransformationFunctions(s);
points = outFunc(points);

% check nargin of expression and adjust
if nargin(s.expression) == 1
	values = s.expression( points );
else
	cellpoints = num2cell(points,1);
	values = s.expression(cellpoints{:});
end
