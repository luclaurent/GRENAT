function plm = legendre(l, amu, tflag)
...
%	related to the (un-normalized) associated Legendre functions 
%	P(n,m;x) by:
%
%	        SP(n,m;x) = sqrt( 2*(n-m)!/(n+m)! ) * P(n,m;x)
% 
%	Example:
...
%
%		P(n,m;x) = ((-1)^m)*sqrt((n+m)!/(n-m)! ) * SPc(n,m;x)
%	
...
% column-by-column recursion
    for k = 1: length(m1)
        ...
        if amu(col) < 0, tlm(mm1,col)=sign(rem(l+1,2)-0.5)*tstart; end;
...
if strcmp(tflag,'sch')
    % Calculate the standard Schmidt semi-normalized functions from these
    % functions. For m = 1,...,l, multiply by (-1)^m*sqrt(2)
    row1 = plm(1,:);
    plm = sqrt(2)*plm;
    plm(1,:) = row1;	% restore first row
    const1 = 1;
    for r = 2: l+1;
        const1 = -const1;
        plm(r,:) = const1*plm(r,:);
    ...
    % Calculate the standard A&S functions (i.e., unnormlized case) from 
    % these functions by multiplying each row by: ((-1)^m)*sqrt((l+m)!/(l-m)!)
    const1 = 1;
    for r = 2: l+1
        m = r-1;
