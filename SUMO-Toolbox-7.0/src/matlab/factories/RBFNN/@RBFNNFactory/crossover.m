...
for i=1:nKids
	...
	spread1 = getSpread(parent1);
	spread2 = getSpread(parent2);

	spread3 = (spread1/2 + spread2/2) + rand;

	model = s.createModel(spread3);
...
s.logger.fine(sprintf('Produced %d of %d rbfnn crossover children',nKids,length(thisPopulation)));
