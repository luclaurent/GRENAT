...
for i=1:length(parents)
	...
	spread = boundedRand(lb,ub);
	
	model = s.createModel(spread);
	mutationChildren{i,1} = model;
end

% Make sure all are trained
mutationChildren = constructModels(mutationChildren,smp,val,s.getParallelMode());

s.logger.fine(sprintf('Produced %d of %d rbfnn mutation children',length(parents),length(thisPopulation)));
