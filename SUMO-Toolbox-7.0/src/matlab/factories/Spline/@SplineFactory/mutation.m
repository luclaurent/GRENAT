...
for i = 1:length(parents)
	...
	[lb ub] = getBounds(s);
	smoothing = boundedRand(lb,ub);

	sm = s.createModel(smoothing);
...
s.logger.fine(sprintf('Produced %d of %d spline mutation children',length(parents),length(thisPopulation)));
