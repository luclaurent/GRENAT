...
fprintf('>>> Preparation donnees RBF a gradient indirect (appel RBF)  \n');
...
if ~isstruct(grad)
    ...
    %creation d�calage par direction
    ...
    if manq.eval.on
        ...
        fprintf(' >>> Supression informations (reponse(s) manquante(s)) a (aux) point(s):');
        fprintf(' %i',pos_ev);
        fprintf('\n');
