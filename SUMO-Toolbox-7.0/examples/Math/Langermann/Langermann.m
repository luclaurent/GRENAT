function out = Langermann(varargin)
...
%
% Signature:
%	out = Langermann(varargin)
%
% Description:
%	The langermann function in Matlab code, for up to 10 inputs.
...
% scale input array from [-1,1] to [0,10]
x = (x + 1) .* 5;

% dimensionality must not exceed 10
if n > 10
	disp('ERROR: Langermann only supports up to 10 dimensions');
	out = [];
	return;
end
...
for k=1:size(x,1)
	...
	Sum = 0;
	for i = 1 : 5
		dist = SqrDst(p, A(i,1:n), n);
		Sum = Sum - (c(i) * (exp(-dist/pi) * cos(pi * dist)));
	end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
function res = SqrDst(x1, x2, n)
	res = sum((x1(1:n) - x2(1:n)) .^ 2);
