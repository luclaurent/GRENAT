%% Fonction: multiquadratics (RBF)
%% L. LAURENT -- 17/01/2012 (r: 31/08/2015) -- luc.laurent@cnam.fr

function [G,dG,ddG]=invmultiqua(xx,long)
...
elseif nargout==2
    G=ev;
    dG=-1/long.*xx./repmat(ev,1,nb_comp).^3;
elseif nargout==3
    G=ev;
    dG=-1/long.*xx./repmat(ev,1,nb_comp).^3;   
    ...
    if pt_eval==1
        ...
        for ll=1:nb_comp
           for mm=1:nb_comp
                if(mm==ll)
                    ddG(mm,ll)=-1/long*(ev^3-3*xx(mm)^2/long*ev^5);
                else
                    ddG(mm,ll)=3*xx(ll)*xx(mm)/long^2*ev^5;
                end
           end
        end
    ...
    error('Mauvais argument de sortie de la fonction invmultiqua.m');
end