...
if model.y_dim>1,
  if (length(model.kernel_pars)==model.y_dim | size(model.gam,2)==model.y_dim |prod(size(model.kernel_type,2))==model.y_dim)
    ...
    if (nargout>1),
      ...
      model.duration = etime(clock,startTime);
...
if strcmpi(model.implementation,'CMEX'),
  ...
  eval(['[model.alpha, model.b,model.cga_startvalues] =' ...
	...
	'model.cga_show,dyn_pars);'],...
        'model.implementation=''MATLAB'';');% if error in CMEX ...

% Edit by Dirk Gorissen : if error in CMEX (mex files not compiled) go straight to Matlab, skip CFILE
% Since there is no real point and causes file problems in some cases.
...
if (nargout>1),
 ...
 %Removed toc since it interferes with other tic/tocs
 %model.duration = toc;
  model.duration = 0;
  model.status = 'trained';
end
