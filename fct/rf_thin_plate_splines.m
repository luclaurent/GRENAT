...
function [rf,drf,ddrf]=rf_thin_plate_splines(xx,long)
...
elseif nargout==2
    rf=ev;
    drf=xx./repmat(ev,1,nb_comp).^(para-2).*(1+para*repmat(le,1,nb_comp));
elseif nargout==3
    rf=ev;
    drf=xx./repmat(ev,1,nb_comp).^(para-2).*(1+para*repmat(le,1,nb_comp));
    ...
    if pt_eval==1
        ...
        for ll=1:nb_comp
           for mm=1:nb_comp
                if(mm==ll)
                    ddrf(mm,ll)=ev^(para-2)*(1+para*le)+2*(para-1)*xx(mm)^2*...
                        ev^(para-4)+xx(mm)*para*(para-2)*ev^(para-4)*le;
                else
                    ddrf(mm,ll)=xx(ll)*xx(mm)*ev^(para-4)*...
